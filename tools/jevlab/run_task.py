...
def run(args: argparse.Namespace) -> int:
    ...
    embed = np.load(root / "cache" / args.task / f"embed_i{fly_cfg.inject_count}_seed{fly_cfg.seed}.npy")
    ...
    fly_lat = packet_latency_ms(tok, res, head, texts, pooling=fly_cfg.pooling, n=min(50, len(texts)), seed=0)
    nofly_lat = nofly_latency_ms(tok, embed, texts, pooling=nofly_cfg.pooling, n=min(50, len(texts)), seed=0)
    latency = {
    ...
    if not index.exists():
        index.write_text("run_dir\ttask\tpass\tpublishable\tstarted\tfinished\n", encoding="utf-8")
