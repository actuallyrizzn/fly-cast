...
A profile is a folder: `profile.toml` plus fixtures. The library loads it; domain content lives with the profile.
...
| Lexicon | `phrase <TAB> weight` — lowercase; higher weight → soft bonus on matching candidates |
...
Shared mouth changes go in `src/flycast/`. Profile work stays under `examples/`.
